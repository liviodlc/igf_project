...
/**
 * The base class that all items must extend in order
 * to be rendered in the game
 * Note that some extensions of this class may not necessarily need 
 * the hardwired vis value (and in fact will have more than one vis range)
 * 
 * @author Teresa
 *
 */
...
//	}
	...
	protected BufferedImage blend(BufferedImage b1, BufferedImage b2, double weight){
		...
		BufferedImage blended = new BufferedImage(b1.getWidth(), b1.getHeight(), BufferedImage.TYPE_INT_RGB);
		Graphics2D g2 = blended.createGraphics();
		
		//draw the new image w/appropriate blending
		g2.drawImage(b1, null, 0, 0);
		g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,(float)(1.0-weight)));
